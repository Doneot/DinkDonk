...
async function bootstrap() {
  ...
  const repository = new FirestoreRepository(firestore, {
    onUserChanged: (userId, user) => sockets.notifyUser(userId, 'user_data_updated', user),
  });
  ...
  const discord = new DiscordBot({
    ...
    context,
    onDmCapabilityChanged: (userId, canReceiveDM) => repository.saveUser(userId, { canReceiveDM }),
  });
  context.discord = discord;

  const notificationService = new NotificationService({ twitch, repository, discord });
  const app = createApp({ firestore, repository, twitch, discord, notificationService });
  httpServer.on('request', app);

  repository.on('streamerAdded', (streamerId) => notificationService.handleStreamerAdded(streamerId));
  repository.on('streamerEmpty', (streamerId) => notificationService.garbageCollectStreamer(streamerId));
  twitch.on('ready', () => notificationService.syncEventSubSubscriptions().then(() => notificationService.garbageCollectSubscriptions()));
  twitch.on('tokenRefreshed', () => notificationService.syncEventSubSubscriptions());
  ...
  const shutdown = async (signal) => {
    ...
    try {
      await sockets.close();
      await discord.stop();
      await twitch.stop({ unsubscribeEventSub: env.unsubscribeEventSubOnShutdown });
